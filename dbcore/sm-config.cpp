...
bool hbm_arch = false;
uint32_t max_numa_node = 0;
uint32_t scheduling_policy = 1;
uint64_t arrival_interval_us = 1000000;
uint32_t main_queue_size = 1;
uint32_t prioritized_queue_size = 1;
uint32_t global_queue_size = 1;
uint32_t max_preempted_cycle_pct = 100;
uint32_t switch_threshold = 10;
uint32_t latency_sample_rate = 0;
uint32_t latency_sample_size = 100000;
...
void init() {
  ...
  if (hbm_arch) {
    max_numa_node = numa_max_node() / 2;
  } else {
    max_numa_node = numa_max_node();
  }

  if (numa_spread) {
    numa_nodes = threads > max_numa_node + 1 ? max_numa_node + 1 : threads;
  } else {
    uint32_t max = thread::cpu_cores.size() / (max_numa_node + 1);
