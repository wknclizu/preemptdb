...
thread_local struct io_uring tls_read_ring; // TODO(pcontext): context-local rings
...
void tls_log::issue_read(int fd, char *buf, uint64_t size, uint64_t offset, void *user_data) {
  thread_local bool initialized = false; // TODO(pcontext): we need a separate ring for each pcontext
