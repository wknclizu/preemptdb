...
thread_local context_data *cls = nullptr;

/* Used to make sure threads give back their caches on exit */
std::mutex oid_mutex;
sm_oid_mgr *master;
std::map<uint64_t, context_data *> *contexts;
 ...
 */
void context_revoke_caches(sm_oid_mgr *om) {
  for (auto it = cls->caches.begin(); it != cls->caches.end(); ++it) {
    pcontext::lock();
    om->lock_file(it->f);
    DEFER({om->unlock_file(it->f); pcontext::unlock();});
  ...
  pcontext::lock();
  cls->mutex.lock();
  DEFER({cls->mutex.unlock(); pcontext::unlock();});
  cls->caches.clear();
}

void context_fini(sm_oid_mgr *om) {
  auto cid = pthread_self();
  context_revoke_caches(om);

  pcontext::lock();
  oid_mutex.lock();
  DEFER({oid_mutex.unlock(); pcontext::unlock();});
...
/* Find (or create, if necessary) the thread-local cache for the
   requested FID
 */
context_data::cache_map::iterator context_cache(sm_oid_mgr *om, FID f) {
  pcontext::lock();
  if (not cls) {
    context_init();
  }
   ...
   */
  auto rval = cls->caches.find_and_emplace(f, f);
  if (rval.second == -1) {
     ...
     */
    context_revoke_caches(om);
    rval = cls->caches.find_and_emplace(f, f);
...
OID context_allocate(sm_oid_mgr *om, FID f) {
  // correct cache entry definitely exists now... but may be empty
  auto it = context_cache(om, f);
  ASSERT(it->f == f);
  if (not it->nentries) {
    pcontext::lock();
    om->lock_file(f);
    DEFER({om->unlock_file(f); pcontext::unlock();});
...
void context_free(sm_oid_mgr *om, FID f, OID o) {
...
sm_oid_mgr::~sm_oid_mgr() {
  ...
  */
  ...
  for (auto &kv : *contexts) {
    ...
    pcontext::lock();
    tdata->mutex.lock();
    DEFER({tdata->mutex.unlock(); pcontext::unlock();});
...
FID sm_oid_mgr::create_file(bool needs_alloc) {
   ...
   */
  auto f = context_allocate(this, OBJARRAY_FID);
...
void sm_oid_mgr::recreate_allocator(FID f, OID m) {
  ...
  pcontext::lock();
  recreate_lock.lock();
  DEFER({recreate_lock.unlock(); pcontext::unlock();});
...
void sm_oid_mgr::destroy_file(FID f) {
  ...
  context_free(this, OBJARRAY_FID, f);
}
 ...
 */
alloc:
  auto o = context_allocate(this, f);
