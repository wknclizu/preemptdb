...
extern xid_context xid_contexts[NCONTEXTS];
...
inline XID take_one(context_data *ctx_data) {
  ASSERT(ctx_data->bitmap);
  auto old_bitmap = __sync_fetch_and_and(&ctx_data->bitmap, (ctx_data->bitmap - 1));
  auto id = ctx_data->base_id + __builtin_ctzll(old_bitmap);
  auto x = xid_contexts[id].owner = XID::make(ctx_data->epoch, id);
#ifdef SSN
  xid_contexts[id].sstamp = 0;
  xid_contexts[id].pstamp = 0;
  xid_contexts[id].xct = NULL;
#endif
#ifdef SSI
  xid_contexts[id].ct3 = 0;
  xid_contexts[id].last_safesnap = 0;
  xid_contexts[id].xct = NULL;
#endif
  // Note: transaction needs to initialize xc->begin in ctor
  xid_contexts[id].end = 0;
  ASSERT(xid_contexts[id].state != TXN_COMMITTING);
  xid_contexts[id].state = TXN_ACTIVE;
  xid_contexts[id].xct = nullptr;
  return x;
}
