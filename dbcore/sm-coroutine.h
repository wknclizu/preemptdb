...
class tcalloc {
   ...
   public:
    ...
    void free_(void *p, size_t byte_size) {
...
extern thread_local tcalloc coroutine_allocator; // TODO(pcontext?): leave it as-is for now until we need to support coroutine
...
template <typename T = void> struct [[nodiscard]] generator {
  ...
  struct promise_type {
    ...
    void operator delete(void *p, size_t sz) { coroutine_allocator.free_(p, sz); }
