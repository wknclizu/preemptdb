...
struct epoch_mgr {
  ...
  struct __attribute__((aligned(8))) cls_storage {
    enum { SIZE = 32 };
    char data[SIZE];
  };

  static_assert(std::is_pod<cls_storage>::value,
                "epoch_mgr::cls_storage must be POD");
  ...
  struct callbacks {
    ...
    */
    ...
    /* Called whenever a thread deregisters itself from this epoch
       ...
       previous call to context_registered and will be forgotten by
    ...
    */
    ...
    /* Called once for each registered thread when an epoch
       ...
       context_registered(). Return a replacement for the epoch
       cookie.
  ...
  epoch_num context_enter();
  bool context_is_active();
  epoch_num context_quiesce();
  void context_exit();
