...
struct Thread {
  ...
  bool is_physical;
  uint64_t fs_base_register;
  uint64_t gs_base_register;

  std::condition_variable trigger;
  std::mutex trigger_lock;

  Thread* shadow_thread;

  Thread();
  Thread(uint16_t n, uint16_t c, uint32_t sys_cpu, bool is_physical, bool has_shadow_thread);
  ~Thread();

  static void* PreemptiveStack();
  static uint64_t PreemptiveStackStart();
  static uint64_t PreemptiveStackEnd();
  static pcontext* MainContext();
  static pcontext* MainContext(uint32_t id);
  static pcontext* PreemptiveContext();
