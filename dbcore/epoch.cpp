...
struct epoch_mgr::context_state {
  epoch_num begin;
  epoch_num end;
  bool straggler;
  bool initialized;
...
static_assert(std::is_pod<epoch_mgr::context_state>::value, "epoch_mgr::context_state must be POD");
...
struct epoch_mgr::private_state {
  ...
  std::map<uint64_t, context_state *> contexts;
...
static epoch_mgr::context_state *get_cls(epoch_mgr *em) {
  return (epoch_mgr::context_state *)em->cb.get_cls(em->cb.cookie)->data;
}
...
static void straggler_check(epoch_mgr *em, epoch_mgr::context_state *self, epoch_mgr::epoch_num tmp) {
...
bool epoch_mgr::new_epoch() {
  ...
  context_state *stragglers[state->contexts.size()];
  ...
  for (auto &entry : state->contexts) {
    ...
    /* Epoch N-1 is transitioning to cold status. Flag any
       ...
       Straggler contexts have active transactions that started
...
void epoch_mgr::context_init() {
  auto *self = get_cls(this);
  DIE_IF(self->initialized, "Context already registered");

  auto cid = pthread_self();
  ELOG("Registering context %lld with epoch manager %p\n", (long)(uintptr_t)cid, this);
  ...
  state->contexts.insert(std::make_pair(cid, self));
  DEFER_UNLESS(success, state->contexts.erase(cid));

  self->cookie = cb.context_registered(cb.cookie);
  XDEFER_UNLESS(success, cb.context_deregistered(cb.cookie, self->cookie));
...
void epoch_mgr::context_fini() {
  DIE_IF(not context_initialized(), "Context not registered");

  ELOG("Deregistering context %ld with RCU\n", (long)(uintptr_t)pthread_self());
  ...
  cb.context_deregistered(cb.cookie, self->cookie);
  self->cookie = NULL;
  self->initialized = false;
  // remove us from the system
  auto found = state->contexts.erase(pthread_self());
  ASSERT(found);

  if (not state->contexts.empty()) return;
...
bool epoch_mgr::context_initialized() { return get_cls(this)->initialized; }

epoch_mgr::epoch_num epoch_mgr::context_enter() {
  // FIXME(khuang): tls for preemptive context
  DIE_IF(not context_initialized(), "Context not initialized");
  DIE_IF(context_is_active(), "Context already active");
...
bool epoch_mgr::context_is_active() {
  auto *self = get_cls(this);
  return self->begin > self->end;
}

epoch_mgr::epoch_num epoch_mgr::context_quiesce() {
  DIE_IF(not context_is_active(), "Context not currently active");
  auto *self = get_cls(this);
