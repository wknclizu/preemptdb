...
DEFINE_string(tpcc_workload_mix, "", "");
DEFINE_string(tpcc_prioritized_workload_mix, "", "");
...
unsigned g_preemptive_txn_workload_mix[8] = {
    0, 0, 0, 0, 0, 0, 0, 0};  // default TPC-CH prioritized workload mix
...
void tpcc_parse_options() {
  ...
  unsigned workload_mix_sum = 0;
  if (!gflags::GetCommandLineFlagInfoOrDie("tpcc_workload_mix").is_default) {
    const std::vector<std::string> toks = util::split(FLAGS_tpcc_workload_mix, ',');
    ...
    ALWAYS_ASSERT(workload_mix_sum == 100);
  }

  workload_mix_sum = 0;
  if (!gflags::GetCommandLineFlagInfoOrDie("tpcc_prioritized_workload_mix").is_default) {
    const std::vector<std::string> toks = util::split(FLAGS_tpcc_prioritized_workload_mix, ',');
    ALWAYS_ASSERT(toks.size() == ARRAY_NELEMS(g_preemptive_txn_workload_mix));
    for (size_t i = 0; i < toks.size(); i++) {
      unsigned p = strtoul(toks[i].c_str(), nullptr, 10);
      ALWAYS_ASSERT(p >= 0 && p <= 100);
      workload_mix_sum += p;
      g_preemptive_txn_workload_mix[i] = p;
    }
    ALWAYS_ASSERT(workload_mix_sum == 100 || workload_mix_sum == 0);
  }
  ...
  if (ermia::config::verbose) {
    ...
    std::cerr << "  workload mix                 : "
         << util::format_list(g_txn_workload_mix,
                        g_txn_workload_mix + ARRAY_NELEMS(g_txn_workload_mix))
         << std::endl;
    std::cerr << "  prioritized workload mix                 : "
         << util::format_list(g_preemptive_txn_workload_mix,
                        g_preemptive_txn_workload_mix + ARRAY_NELEMS(g_preemptive_txn_workload_mix))
         << std::endl;
