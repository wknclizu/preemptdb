...
DEFINE_string(ycsb_read_tx_type, "sequential", "Type of read transaction: sequential.");
...
void ycsb_table_loader::do_load(ermia::OrderedIndex *tbl, std::string table_name, uint64_t hot_record_count, uint64_t cold_record_count) {
  uint32_t nloaders = std::thread::hardware_concurrency() / (ermia::config::max_numa_node + 1) / 2 * ermia::config::numa_nodes;
