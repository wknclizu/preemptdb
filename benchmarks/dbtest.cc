...
DEFINE_bool(hbm_arch, false, "Whether the system has HBM");
DEFINE_uint32(scheduling_policy, 0, "scheduling policy, 0: no queue, 1: vanilla, 2: preemptive, 3: yield");
DEFINE_uint64(arrival_interval_us, 1000000, "Arrival interval (microsecond) of high priority transactions");
DEFINE_uint32(main_queue_size, 1, "Number of transactions admitted to each worker");
DEFINE_uint32(prioritized_queue_size, 1, "Number of transactions admitted to each worker");
DEFINE_uint32(global_queue_size, 1, "Size of global queue");
DEFINE_uint32(max_preempted_cycle_pct, 100, "Max percentage of preempted cycles per main context");
DEFINE_uint32(switch_threshold, 10, "Switch to preemptive context every #switch_threshold record access");
DEFINE_uint32(latency_sample_rate, 0, "Sample rate in percent [0,100] for latency measurement");
DEFINE_uint32(latency_sample_size, 100000, "Reservoir size for latency measurement");
...
void bench_main(int argc, char **argv, std::function<void(ermia::Engine *)> test_fn) {
  ...
  ermia::config::hbm_arch = FLAGS_hbm_arch;
  if (FLAGS_hbm_arch) {
    ermia::config::max_numa_node = numa_max_node() / 2;
  } else {
    ermia::config::max_numa_node = numa_max_node();
  }
  ermia::config::scheduling_policy = FLAGS_scheduling_policy;
  ermia::config::arrival_interval_us = FLAGS_arrival_interval_us;
  ermia::config::main_queue_size = FLAGS_main_queue_size;
  ermia::config::prioritized_queue_size = FLAGS_prioritized_queue_size;
  ermia::config::global_queue_size = FLAGS_global_queue_size;
  ermia::config::max_preempted_cycle_pct = FLAGS_max_preempted_cycle_pct;
  ermia::config::switch_threshold = FLAGS_switch_threshold;
  ermia::config::latency_sample_rate = FLAGS_latency_sample_rate;
  ermia::config::latency_sample_size = FLAGS_latency_sample_size;
  ...
  if (ermia::config::latency_sample_rate)
    std::cerr << "  latency sample rate (%): " << ermia::config::latency_sample_rate << std::endl;
  if (ermia::config::latency_sample_size)  
    std::cerr << "  latency sample size: " << ermia::config::latency_sample_size << std::endl;
  switch (ermia::config::scheduling_policy) {
    case 1:
      std::cerr << "  scheduling-policy: vanilla" << std::endl;
      break;
    case 2:
      std::cerr << "  scheduling-policy: preemptive" << std::endl;
      break;
    case 3:
      std::cerr << "  scheduling-policy: yield" << std::endl;
      break;
    default:
      break;
  }
  std::cerr << "  arrival-interval-us : " << ermia::config::arrival_interval_us << std::endl;
  std::cerr << "  main-queue-size: " << ermia::config::main_queue_size << std::endl;
  std::cerr << "  prioritized-queue-size: " << ermia::config::prioritized_queue_size << std::endl;
  std::cerr << "  global-queue-size: " << ermia::config::global_queue_size << std::endl;
  std::cerr << "  max-preempted-cycle-percentage: " << ermia::config::max_preempted_cycle_pct << std::endl;
  std::cerr << "  switch-threshold: " << ermia::config::switch_threshold << std::endl;
