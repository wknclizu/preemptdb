...
typedef std::tuple<uint64_t, uint64_t, uint64_t, uint64_t, uint64_t, std::vector<uint64_t>, uint64_t, uint64_t, uint64_t, uint64_t> tx_stat;
...
class bench_worker : public ermia::thread::Runner {
 public:
  bench_worker(unsigned int worker_id, bool is_worker, unsigned long seed,
               ...
               spin_barrier *barrier_a = nullptr, spin_barrier *barrier_b = nullptr, ermia::thread::Thread *_me = nullptr)
      :         worker_id(worker_id),
        is_worker(is_worker),
        r(seed),
        db(db),
    ...
    random_bool = std::bind(std::bernoulli_distribution((double)ermia::config::latency_sample_rate/100), std::knuth_b());
  }
  ~bench_worker() {}
  ...
  struct workload_desc {
    ...
    workload_desc(const std::string &name, double frequency, txn_fn_t fn,
                  coro_txn_fn_t cf=nullptr, task_fn_t tf=nullptr, bool p=false)
        : name(name), frequency(frequency), fn(fn), coro_fn(cf) , task_fn(tf), preemptive(p) {
  ...
  std::function<bool()> random_bool;
  bench_worker *preemptive_worker;
  uint16_t preemptive_workload_idx;
  OneToOneQueue<std::pair<uint32_t, util::timer>> main_workload_queue;
  OneToOneQueue<std::pair<uint32_t, util::timer>> preemptive_workload_queue;
  OneToOneQueue<std::pair<uint32_t, util::timer>> global_workload_queue;
  ...
  inline uint64_t get_latency_number_us() const {
    return ermia::volatile_read(latency_number_us);
  }

  inline double get_avg_latency_us() const {
    return double(latency_number_us) / double(ntxn_commits);
  }

  const tx_stat_map get_txn_counts() const;

  void do_workload_function(uint32_t i);
  void do_workload_function(uint32_t i, util::timer &t, bool accumulate_cycle=false);
  void do_preemptive_transaction();
  static void static_preemptive_transaction();

  bool has_prioritized_workload();
  uint32_t fetch_workload();
  uint32_t fetch_preemptive_workload();
 ...
 protected:
  ...
  mutable uint16_t regular_workload_size;
};
...
class bench_runner {
 public:
  ...
  bench_runner(ermia::Engine *db)
      : db(db),
        barrier_a(ermia::config::scheduling_policy ? ermia::config::worker_threads + 1
