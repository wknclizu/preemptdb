...
#ifndef NESTED_COROUTINE
#ifndef HYBRID_COROUTINE
...
start_over:
    ...
    handle_visible:
      ...
      co_return t->DoTupleRead(cur_obj->GetPinnedTuple(t), &value);
    ...
    case mystack_type::scan_emit: { // surpress cross init warning about v
      ...
      get_version_start_over:
        ...
        while (ptr.offset()) {
          ...
          if (visible) {
            if (!scanner.visit_value(ka, cur_obj->GetPinnedTuple(t))) {
              goto done;
            }
            break;
          }
