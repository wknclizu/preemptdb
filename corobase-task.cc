...
ConcurrentMasstreeIndex::task_GetRecord(transaction *t, const varstr &key,
                                        varstr &value, OID *out_oid) {
  OID oid = INVALID_OID;
  rc_t rc = rc_t{RC_INVALID};

  // start: masstree search
  ConcurrentMasstree::contextinfo ti(t->xc->begin_epoch);
...
forward:
  ...
  start_over:
    ...
    if (ptr.asi_type() == fat_ptr::ASI_LOG) {
      ...
      dbtuple *tuple = co_await Object::TaskLoadFromStorage(visitor_xc->xct, ptr, nullptr);
      if (!tuple) {
        found = false;
      }
...
ermia::coro::task<rc_t> ConcurrentMasstreeIndex::task_UpdateRecord(transaction *t, const varstr &key, varstr &value) {
...
// end: masstree search
  ...
  check_prev:
    ...
    if (prev_obj) {  // succeeded
      ...
      co_await suspend_always{};
      dbtuple *tuple = ((Object *)new_obj_ptr.offset())->GetPinnedTuple(t);
      ASSERT(tuple);
      dbtuple *prev = prev_obj->GetPinnedTuple(t);
...
ermia::coro::task<rc_t> ConcurrentMasstreeIndex::task_InsertRecord(transaction *t, const varstr &key, varstr &value, OID *out_oid) {
  // For primary index only
  ALWAYS_ASSERT(IsPrimary());
...
// strat: insert_if_absent
  ...
  if (t->xc)
    e = t->xc->begin_epoch;
  ConcurrentMasstree::contextinfo ti(e);
...
ermia::coro::task<bool> ConcurrentMasstreeIndex::task_InsertOID(transaction *t, const varstr &key, OID oid) {
  ASSERT((char *)key.data() == (char *)&key + sizeof(varstr));
...
ermia::coro::task<rc_t> ConcurrentMasstreeIndex::task_Scan(transaction *t,
    ...
    case mystack_type::scan_emit: { // surpress cross init warning about v
      ...
      get_version_start_over:
        ...
        while (ptr.offset()) {
          ...
          if (visible) {
            if (!scanner.visit_value(ka, cur_obj->SyncGetPinnedTuple(t))) {
              goto done;
            }
            break;
          }
