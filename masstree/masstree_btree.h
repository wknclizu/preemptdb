...
class simple_contextinfo {
public:
  simple_contextinfo(epoch_num e) : ts_(0), epoch_(e) {}
  class rcu_callback {
  public:
    virtual void operator()(simple_contextinfo &ti) = 0;
...
struct masstree_params : public Masstree::nodeparams<> {
  ...
  typedef simple_contextinfo contextinfo_type;
};
...
public:
  ...
  search_coro(const key_type &k, OID &o, contextinfo &ti,
              versioned_node_t *search_info = nullptr) const;

  inline ermia::coro::task<bool>
  search_task(const key_type &k, OID &o, contextinfo &ti,
              versioned_node_t *search_info = nullptr) const;
...
public:
  ...
  public:
   ...
   PROMISE(bool) init_or_next() {
       contextinfo ti(xc_->begin_epoch);
  ...
  public:
    ...
    ermia::coro::generator<bool> init(bool emit_firstkey=true) {
      contextinfo ti(this->xc_->begin_epoch);
...
template <typename P>
inline ermia::coro::generator<bool>
mbtree<P>::search_coro(const key_type &k, OID &o, contextinfo &ti,
                       versioned_node_t *search_info) const {
  Masstree::unlocked_tcursor<P> lp(table_, k.data(), k.size());
...
template <typename P>
inline ermia::coro::task<bool>
mbtree<P>::search_task(const key_type &k, OID &o, contextinfo &ti,
                       versioned_node_t *search_info) const {
  Masstree::unlocked_tcursor<P> lp(table_, k.data(), k.size());
...
public:
  ...
  inline void visit_leaf(const Masstree::scanstackelt<P> &iter,
                  const Masstree::key<uint64_t> &key, contextinfo &) {
    this->n_ = iter.node();
